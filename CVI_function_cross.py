...
def false():  # функция сопоставления файлов и вывода повторяющихся строк
    for x in range(b, num):  # цикл открытия всех файлов
        a = xlrd.open_workbook('E:\code\Cvi\Cvi2\\' + files[x], on_demand=True)
...
files = os.listdir('E:\code\Cvi\Cvi2') # создание списка из названий файлов
#for file in files:
...
for i in range (num): # цикл открытия всех файлов
    a = xlrd.open_workbook('E:\code\Cvi\Cvi2\\' + files[i], on_demand=True)
    b+=1
