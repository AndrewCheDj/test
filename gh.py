...
def false():  # функция сопоставления файлов и вывода повторяющихся строк
    for x in range(b, num):  # цикл открытия всех файлов
        a = xlrd.open_workbook('E:\code\Cvi\Cvi3\\' + files[x], on_demand=True)
        ...
        for rownum in range(sheet.nrows):  # извлечение данных из файла
            ...
            if rownum > 6:
                ...
                if SecondString == FirstString:
                    print (' В файле ' + str(files[x]) + ' в строке ' + str(rownum+1) + ' замечен хуй из файла ' + str(files[0]) + ' строка ' + str(rownumGlobal+1))
                else:
                    z=1


# выбор директории для работы
files = os.listdir('E:\code\Cvi\Cvi3') # создание списка из названий файлов
#for file in files:
...
a = xlrd.open_workbook('E:\code\Cvi\Cvi3\\' + files[0], on_demand=True)
b+=1
